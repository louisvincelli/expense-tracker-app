...
import { collection, connectFirestoreEmulator, deleteDoc, doc, getDoc, getDocs, orderBy, query, setDoc, Timestamp, updateDoc, where } from "firebase/firestore";
...
export const createOrUpdateTransaction = async (
    ...
    try {
        ...
        if(id){
            //update existing transaction based on 3 params
...
const revertAndUpdateWallets = async (
    ...
    try {
        ...
        // or if user tries to increase the expense amount and doesn't have enough balance
            if(
        ...
        const stats = monthlyData.flatMap((month)=> [
            {
              ...
              { value: month.expense, frontColor: colors.rose }, // expense bar color
        ]);
        ...
        const stats = yearlyData.flatMap((year: any)=> [
            {
              ...
              { value: year.expense, frontColor: colors.rose }, // expense bar color
        ]);
